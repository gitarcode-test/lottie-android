...
public class LottieDrawable extends Drawable implements Drawable.Callback, Animatable {
  ...
  /**
   * Internal record keeping of the desired play state when {@link #isVisible()} transitions to or
   * is false.
   *
   * <p>If the animation was playing when it becomes invisible or play/pause is called on it while
   * it is invisible, it will store the state and then take the appropriate action when the drawable
  ...
  /**
   * Prior to Oreo, you could only call invalidateDrawable() from the main thread. This means that
   * when async updates are enabled, we must post the invalidate call to the main thread. Newer
   * devices can call invalidate directly from whatever thread asyncUpdates runs on.
   */
  private static final boolean invalidateSelfOnMainThread =
      Build.VERSION.SDK_INT <= Build.VERSION_CODES.N_MR1;

  /**
   * The marker to use if "reduced motion" is enabled. Supported marker names are case insensitive,
   * and include: - reduced motion - reducedMotion - reduced_motion - reduced-motion
   */
  private static final List<String> ALLOWED_REDUCED_MOTION_MARKERS =
      Arrays.asList("reduced motion", "reduced_motion", "reduced-motion", "reducedmotion");
  ...
  @Nullable private Map<String, Typeface> fontMap;

  /**
   * Will be set if manually overridden by {@link #setDefaultFontFileExtension(String)}. This must
   * be stored as a field in case it is set before the font asset delegate has been created.
   */
  ...
  /**
   * True if the drawable has not been drawn since the last invalidateSelf. We can do this to
   * prevent things like bounds from getting recalculated many times.
   */
  ...
  /**
   * Ensures that setProgress and draw will never happen at the same time on different threads. If
   * that were to happen, parts of the animation may be on one frame while other parts would be on
   * another.
   */
   ...
   *
   * <p>Defaults to a core size of 0 so that when no animations are playing, there will be no idle
   * cores consuming resources.
   *
   * <p>Allows up to two active threads so that if there are many animations, they can all work in
   * parallel. Two was arbitrarily chosen but should be sufficient for most uses cases. In the case
   * of a single animation, this should never exceed one.
   *
   * <p>Each thread will timeout after 35ms which gives it enough time to persist for one frame, one
   * dropped frame and a few extra ms just in case.
   */
  private static final Executor setProgressExecutor =
      new ThreadPoolExecutor(
          0, 2, 35, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(), new LottieThreadFactory());
  ...
  /**
   * Enable this to get merge path support for devices running KitKat (19) and above. Deprecated:
   * Use enableFeatureFlag(LottieFeatureFlags.FeatureFlag.MergePathsApi19, enable)
   *
  ...
  public boolean setComposition(LottieComposition composition) {
    ...
    while (it.hasNext()) {
      ...
      // The task should never be null but it appears to happen in rare cases. Maybe it's an
      // oem-specific or ART bug.
   ...
   * the device is pre-Pie. 2) There are more than 4 masks and mattes and the device is pre-Pie.
   * Hardware acceleration is generally faster for those devices unless there are many large mattes
   * and masks in which case there is a lot of GPU uploadTexture thrashing which makes it much
   * slower.
   *
   ...
   * async updates are enabled or not. This is useful when the mode is automatic and you want to
   * know whether automatic is defaulting to enabled or not.
   */
  ...
  private void computeRenderMode() {
    ...
    useSoftwareRendering =
        renderMode.useSoftwareRendering(
  ...
  /**
   * Helper for the async execution path to potentially call setProgress before drawing if the
   * current progress has drifted sufficiently far from the last set progress.
   *
   * @see AsyncUpdates
  ...
  /**
   * This method is used to get the marker for animations when system animations are disabled. It
   * iterates over the list of allowed reduced motion markers and returns the first non-null marker
   * it finds. If no non-null marker is found, it returns null.
   *
   * @return The first non-null marker from the list of allowed reduced motion markers, or null if
  ...
  public void setMinAndMaxFrame(
    ...
    if (composition == null) {
      lazyCompositionTasks.add(
          c -> setMinAndMaxFrame(startMarkerName, endMarkerName, playEndMarkerStartFrame));
      return;
    }
  ...
  public boolean isAnimating() {
    // On some versions of Android, this is called from the LottieAnimationView constructor, before
    // animator was created.
  ...
  /**
   * Tell Lottie that system animations are disabled. When using {@link LottieAnimationView} or
   * Compose {@code LottieAnimation}, this is done automatically. However, if you are using
   * LottieDrawable on its own, you should set this to false when {@link
   * com.airbnb.lottie.utils.Utils#getAnimationScale(Context)} is 0. If the animation is provided a
   * "reduced motion" marker name, they will be shown instead of the first or last frame. Supported
   * marker names are case insensitive, and include: - reduced motion - reducedMotion -
   ...
   * from After Effects. If your images look like they could be represented with vector shapes, see
   * if it is possible to convert them to shape layers and re-export your animation. Check the
   * documentation at <a href="http://airbnb.io/lottie">http://airbnb.io/lottie</a> for more
   * information about importing shapes from Sketch or Illustrator to avoid this.
   */
   ...
   * file.
   *
   * <p>If you change a value in fontMap, create a new map or call {@link #invalidateSelf()}.
   * Setting the same map again will noop.
   */
   ...
   *
   * <p>If you want to set value callbacks for any of these values, it is recommend to use the
  ...
  /**
   * Add an property callback for the specified {@link KeyPath}. This {@link KeyPath} can resolve to
   ...
   *
   * <p>Set the callback to null to clear it.
   */
  ...
  /**
   * Returns the bitmap that will be rendered for the given id in the Lottie animation file. The id
   * is the asset reference id stored in the "id" property of each object in the "assets" array.
   *
   * <p>The returned bitmap could be from: * Embedded in the animation file as a base64 string. * In
   * the same directory as the animation file. * In the same zip file as the animation file. *
   * Returned from an {@link ImageAssetDelegate}. or null if the image doesn't exist from any of
   * those places.
   */
  @Nullable
  ...
  /**
   * Returns the {@link LottieImageAsset} that will be rendered for the given id in the Lottie
   * animation file. The id is the asset reference id stored in the "id" property of each object in
   * the "assets" array.
   *
   * <p>The returned bitmap could be from: * Embedded in the animation file as a base64 string. * In
   * the same directory as the animation file. * In the same zip file as the animation file. *
    ...
    if (imageAssetManager == null) {
      imageAssetManager =
          new ImageAssetManager(
              getCallback(), imageAssetsFolder, imageAssetDelegate, composition.getImages());
    }
  ...
  public boolean setVisible(boolean visible, boolean restart) {
    // Sometimes, setVisible(false) gets called twice in a row. If we don't check
    // wasNotVisibleAlready, we could
  ...
  /**
   * These Drawable.Callback methods proxy the calls so that this is the drawable that is actually
   * invalidated, not a child one which will not pass the view's validateDrawable check.
   */
  @Override
   ...
   *
   * <p>This draws the animation to an internally managed bitmap and then draws the bitmap to the
   * original canvas.
   *
   * @see LottieAnimationView#setRenderMode(RenderMode)
   */
  private void renderAndDrawAsBitmap(Canvas originalCanvas, CompositionLayer compositionLayer) {
    ...
    // Get the canvas clip bounds and map it to the coordinate space of canvas with it's current
    // transform.
    ...
    if (clipToCompositionBounds) {
      // Start with the intrinsic bounds. This will later be unioned with the clip bounds to find
      // the
    ...
    // The bounds are usually intrinsicWidth x intrinsicHeight. If they are different, an external
    // source is scaling this drawable.
    ...
    if (!ignoreCanvasClipBounds()) {
      softwareRenderingTransformedBounds.intersect(
          canvasClipBounds.left,
          canvasClipBounds.top,
          canvasClipBounds.right,
    ...
    if (isDirty) {
      ...
      renderingMatrix.preScale(scaleX, scaleY);
      // We want to render the smallest bitmap possible. If the animation doesn't start at the top
      // left, we translate the canvas and shrink the
      // bitmap to avoid allocating and copying the empty space on the left and top. renderWidth and
      // renderHeight take this into account.
      renderingMatrix.postTranslate(
          -softwareRenderingTransformedBounds.left, -softwareRenderingTransformedBounds.top);
      ...
      // We need to map the rendered coordinates back to the canvas's coordinates. To do so, we need
      ...
      softwareRenderingOriginalCanvasMatrixInverse.mapRect(
          softwareRenderingDstBoundsRectF, softwareRenderingTransformedBounds);
  ...
  private void ensureSoftwareRenderingBitmap(int renderWidth, int renderHeight) {
    if (softwareRenderingBitmap == null
      ...
      isDirty = true;
    } else if (softwareRenderingBitmap.getWidth() > renderWidth
        || softwareRenderingBitmap.getHeight() > renderHeight) {
      // The bitmap is smaller. Take subset of the original.
      softwareRenderingBitmap =
          Bitmap.createBitmap(softwareRenderingBitmap, 0, 0, renderWidth, renderHeight);
      softwareRenderingCanvas.setBitmap(softwareRenderingBitmap);
      isDirty = true;
    }
  }
  ...
  /**
   * When a View's parent has clipChildren set to false, it doesn't affect the clipBound of its
   * child canvases so we should explicitly check for it and draw the full animation bounds instead.
   */
