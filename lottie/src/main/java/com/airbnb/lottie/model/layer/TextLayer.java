...
public class TextLayer extends BaseLayer {
  ...
  @Nullable private BaseKeyframeAnimation<Integer, Integer> colorCallbackAnimation;
  @Nullable private BaseKeyframeAnimation<Integer, Integer> strokeColorAnimation;
  @Nullable private BaseKeyframeAnimation<Integer, Integer> strokeColorCallbackAnimation;
  @Nullable private BaseKeyframeAnimation<Float, Float> strokeWidthAnimation;
  @Nullable private BaseKeyframeAnimation<Float, Float> strokeWidthCallbackAnimation;
  @Nullable private BaseKeyframeAnimation<Float, Float> trackingAnimation;
  @Nullable private BaseKeyframeAnimation<Float, Float> trackingCallbackAnimation;
  @Nullable private BaseKeyframeAnimation<Integer, Integer> opacityAnimation;
  @Nullable private BaseKeyframeAnimation<Float, Float> textSizeCallbackAnimation;
  @Nullable private BaseKeyframeAnimation<Typeface, Typeface> typefaceCallbackAnimation;
  @Nullable private BaseKeyframeAnimation<Integer, Integer> textRangeStartAnimation;
  @Nullable private BaseKeyframeAnimation<Integer, Integer> textRangeEndAnimation;
  @Nullable private BaseKeyframeAnimation<Integer, Integer> textRangeOffsetAnimation;
  ...
  TextLayer(LottieDrawable lottieDrawable, Layer layerModel) {
    ...
    if (textProperties != null
        && textProperties.textStyle != null
        && textProperties.textStyle.color != null) {
    ...
    if (textProperties != null
        && textProperties.textStyle != null
        && textProperties.textStyle.stroke != null) {
    ...
    if (textProperties != null
        && textProperties.textStyle != null
        && textProperties.textStyle.strokeWidth != null) {
    ...
    if (textProperties != null
        && textProperties.textStyle != null
        && textProperties.textStyle.tracking != null) {
    ...
    if (textProperties != null
        && textProperties.textStyle != null
        && textProperties.textStyle.opacity != null) {
    ...
    if (textProperties != null
        && textProperties.rangeSelector != null
        && textProperties.rangeSelector.start != null) {
    ...
    if (textProperties != null
        && textProperties.rangeSelector != null
        && textProperties.rangeSelector.end != null) {
    ...
    if (textProperties != null
        && textProperties.rangeSelector != null
        && textProperties.rangeSelector.offset != null) {
  ...
  /**
   * Configures the [fillPaint] and [strokePaint] used for drawing based on currently active text
   * ranges.
   *
  ...
  private void configurePaint(DocumentData documentData, int parentAlpha, int indexInDocument) {
    ...
    int textRangeOpacity =
        opacityAnimation != null && isIndexInRangeSelection(indexInDocument)
            ? opacityAnimation.getValue()
            : 100;

    // This alpha value needs to be in the range 0 to 255 to be applied to the Paint instances.
    // We map the layer transform's opacity into that range and multiply it by the fractional
    // opacity of the text range and the parent.
    int alpha =
        Math.round(
            (transformOpacity * 255f / 100f) * (textRangeOpacity / 100f) * parentAlpha / 255f);
  ...
  private boolean isIndexInRangeSelection(int indexInDocument) {
    ...
    if (textRangeStartAnimation != null && textRangeEndAnimation != null) {
      // After effects supports reversed text ranges where the start index is greater than the end
      // index.
      // For the purposes of determining if the given index is inside of the range, we take the
      // start as the smaller value.
      int rangeStart =
          Math.min(textRangeStartAnimation.getValue(), textRangeEndAnimation.getValue());
  ...
  private void drawTextWithGlyphs(
    ...
    for (int i = 0; i < textLineCount; i++) {
      ...
      List<TextSubLine> lines =
          splitGlyphTextIntoLines(textLine, boxWidth, font, fontScale, tracking, true);
      for (int j = 0; j < lines.size(); j++) {
        ...
        if (offsetCanvas(canvas, documentData, lineIndex, line.width)) {
          drawGlyphTextLine(
              line.text, documentData, font, canvas, parentScale, fontScale, tracking, parentAlpha);
        }
  ...
  private void drawGlyphTextLine(
      String text,
  ...
  private void drawTextWithFont(
      DocumentData documentData, Font font, Canvas canvas, int parentAlpha) {
    ...
    for (int i = 0; i < textLineCount; i++) {
      ...
      float boxWidth = documentData.boxSize == null ? 0f : documentData.boxSize.x;
      List<TextSubLine> lines =
          splitGlyphTextIntoLines(textLine, boxWidth, font, 0f, tracking, false);
      for (int j = 0; j < lines.size(); j++) {
        TextSubLine line = lines.get(j);
        lineIndex++;
        ...
        if (offsetCanvas(canvas, documentData, lineIndex, line.width)) {
          drawFontTextLine(
              line.text, documentData, canvas, tracking, characterIndexAtStartOfLine, parentAlpha);
        }
  ...
  private boolean offsetCanvas(
      Canvas canvas, DocumentData documentData, int lineIndex, float lineWidth) {
    ...
    if (lottieDrawable.getClipTextToBoundingBox()
        && size != null
        && position != null
        && lineOffset >= position.y + size.y + documentData.size) {
      return false;
    }
  ...
  /**
   * @param characterIndexAtStartOfLine The index within the overall document of the character at
   *     the start of the line
   * @param parentAlpha
   */
  private void drawFontTextLine(
      String text,
      DocumentData documentData,
      Canvas canvas,
      float tracking,
    ...
    for (int i = 0; i < text.length(); ) {
      ...
      drawCharacterFromFont(
          charString, documentData, canvas, characterIndexAtStartOfLine + i, parentAlpha);
  ...
  private void drawCharacterFromFont(
      String character,
      DocumentData documentData,
      Canvas canvas,
      int indexInDocument,
  ...
  public <T> void addValueCallback(T property, @Nullable LottieValueCallback<T> callback) {
    ...
    if (property == LottieProperty.COLOR) {
      ...
      if (callback == null) {
        ...
        colorCallbackAnimation =
            new ValueCallbackKeyframeAnimation<>((LottieValueCallback<Integer>) callback);
      ...
      if (callback == null) {
        ...
        typefaceCallbackAnimation =
            new ValueCallbackKeyframeAnimation<>((LottieValueCallback<Typeface>) callback);
