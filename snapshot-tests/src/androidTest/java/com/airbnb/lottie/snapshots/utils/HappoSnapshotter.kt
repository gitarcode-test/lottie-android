...
class HappoSnapshotter(
    ...
    private val reportNamePrefixes =
        listOf(BuildConfig.GIT_SHA, gitBranch, BuildConfig.VERSION_NAME).filter { it.isNotBlank() }

    // Use this when running snapshots locally.
    // private val reportNamePrefixes = listOf(System.currentTimeMillis().toString()).filter {
    // it.isNotBlank() }
    ...
    private val okhttp by lazy {
        ...
        val ca: X509Certificate =
            context.resources.openRawResource(R.raw.isrgrootx1).use {
    ...
    suspend fun record(bitmap: Bitmap, animationName: String, variant: String) =
        withContext(Dispatchers.IO) {
            ...
            // This is the biggest bottleneck in overall performance. Compress + save can take ~75ms
            // per snapshot.
            ...
            synchronized(snapshots) {
                snapshots +=
                    Snapshot(bucket, key, bitmap.width, bitmap.height, animationName, variant)
            }
    ...
    suspend fun finalizeReportAndUpload() {
        ...
        Log.d(
            L.TAG,
            "Waited ${System.currentTimeMillis() - recordJobStart}ms for recordings to finish saving."
        )
        val json = JsonObject()
        val snaps = JsonArray()
        json.add("snaps", snaps)
        snapshots.forEach { s -> snaps.add(s.toJson()) }
    ...
    private suspend fun upload(reportName: String, json: JsonElement) {
        ...
        val request =
            Request.Builder()
                .addHeader(
                    "Authorization",
                    Credentials.basic(happoApiKey, happoSecretKey, Charset.forName("UTF-8"))
                )
        ...
        if (response.isSuccessful) {
            ...
            throw IllegalStateException(
                "Failed to upload $reportName to Happo. Failed with code ${response.code}. " +
    ...
    private suspend fun OkHttpClient.executeDeferred(request: Request): Response =
