 ...
 */
@Composable fun rememberLottieAnimatable(): LottieAnimatable = remember { LottieAnimatable() }
...
/**
 * [LottieAnimatable] is an extension of [LottieAnimationState] that contains imperative suspend
 * functions to control animation playback.
 *
 * To create one, call:
 * ```
 ...
 *
 * [LottieAnimationState] ensures *mutual exclusiveness* on its animations. To achieve this, when a
 * new animation is started via [animate] or [snapTo], any ongoing animation will be canceled via a
 * [CancellationException]. Because of this, it is possible that your animation will not start
 * synchronously. As a result, if you switch from animating one composition to another, it is not
 * safe to render the second composition immediately after calling animate. Instead, you should
 * always rely on [LottieAnimationState.composition] and [LottieAnimationState.progress].
 *
...
interface LottieAnimatable : LottieAnimationState {
    /**
     * Snap to a specific point in an animation. This can be used to update the progress or
     * iteration count of an ongoing animation. It will cancel any ongoing animations on this state
     * class. To update and then resume an animation, call [animate] again with
     * continueFromPreviousAnimate set to true after calling [snapTo].
     *
     * @param composition The [LottieComposition] that should be rendered. Defaults to
     *   [LottieAnimatable.composition].
     * @param progress The progress that should be set. Defaults to [LottieAnimatable.progress]
     * @param iteration Updates the current iteration count. This can be used to "rewind" or
     *   "fast-forward" an ongoing animation to a past/future iteration count. Defaults to
     *   [LottieAnimatable.iteration]
     * @param resetLastFrameNanos [rememberLottieAnimatable] keeps track of the frame time of the
     *   most recent animation. When [animate] is called with continueFromPreviousAnimate set to
     *   true, a delta will be calculated from the most recent [animate] call to ensure that the
     *   original progress is unaffected by [snapTo] calls in the middle. Defaults to false if
     *   progress is not being snapped to. Defaults to true if progress is being snapped to.
     */
    suspend fun snapTo(
     ...
     * @param iteration The iteration to start the animation at. Defaults to 1 and carries over from
     *   previous animates.
     ...
     * @param speed The speed at which the composition should be animated. Can be negative. Defaults
     *   to 1 and carries over from previous animates.
     * @param clipSpec An optional [LottieClipSpec] to trim the playback of the composition between
     *   two values. Defaults to null and carries over from previous animates.
     * @param initialProgress An optional progress value that the animation should start at.
     *   Defaults to the starting progress as defined by the clipSpec and speed. Because the default
     *   value isn't the existing progress value, if you are resuming an animation, you probably
     *   want to set this to [progress].
     * @param continueFromPreviousAnimate When set to true, instead of starting at the minimum
     *   progress, the initial progress will be advanced in accordance to the amount of time that
     *   has passed since the last frame was rendered.
     * @param cancellationBehavior The behavior that this animation should have when cancelled. In
     *   most cases, you will want it to cancel immediately. However, if you have a state based
     *   transition and you want an animation to finish playing before moving on to the next one
     *   then you may want to set this to [LottieCancellationBehavior.OnIterationFinish].
     * @param ignoreSystemAnimationsDisabled When set to true, the animation will animate even if
     *   animations are disabled at the OS level. Defaults to false.
     * @param useCompositionFrameRate Lottie files can specify a target frame rate. By default,
     *   Lottie ignores it and re-renders on every frame. If that behavior is undesirable, you can
     *   set this to true to use the composition frame rate instead. Note: composition frame rates
     *   are usually lower than display frame rates so this will likely make your animation feel
     *   janky. However, it may be desirable for specific situations such as pixel art that are
     *   intended to have low frame rates.
     */
    suspend fun animate(
...
private class LottieAnimatableImpl : LottieAnimatable {
    ...
    override val isAtEnd: Boolean by derivedStateOf {
        iteration == iterations && progress == endProgress
    }
    ...
    private suspend fun doFrame(iterations: Int): Boolean {
        return if (iterations == LottieConstants.IterateForever) {
            // We use withInfiniteAnimationFrameNanos because it allows tests to add a
            // CoroutineContext
            // element that will cancel infinite transitions instead of preventing composition from
            // ever going idle.
            withInfiniteAnimationFrameNanos { frameNanos -> onFrame(iterations, frameNanos) }
        } else {
            withFrameNanos { frameNanos -> onFrame(iterations, frameNanos) }
        }
    }
    ...
    private fun updateProgress(progress: Float) {
        ...
        this.progress =
            if (useCompositionFrameRate) progress.roundToCompositionFrameRate(composition)
            else progress
    }
}

private fun defaultProgress(
    composition: LottieComposition?,
    clipSpec: LottieClipSpec?,
    speed: Float
): Float {
